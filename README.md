# Hi, I'm Gen Tregea👋 
### Systems Optimiser & Master Data Specialist.

Focused on the intersection of **Supply Chain Architecture** and **Data Remediation**. I specialise in transforming legacy messy data into high-integrity assets for SAP ECC and S/4HANA environments.

### Technical Stack
* **ERP:** SAP ECC 6.0, S/4HANA (Materials Management, Master Data Governance)
* **Data Tools:** Power Query (M), Excel (Advanced), SQL
* **Methodologies:** LSMW, Data Remediation, SKU Rationalisation, Pattern-Matching

### Current Focus
* **Remediation:** Auditing and standardising 10,000+ industrial SKUs
* **Analytics:** Building Supply Chain dashboards in Power BI
* **Learning:** Advanced SAP MDG, Supply Chain Analytics & Data Science pathways.

---
**Portfolio:** [Link pending]

---
**Fun Fact:** Currently completing a 5,000 piece jigsaw puzzle for fun - the pattern matching never stops! 🧩

<!--
**gentregea/gentregea** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
